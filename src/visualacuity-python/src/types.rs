...
macro_rules! pywrap_enum {
    ($name:ident) => {
        impl IntoPy<PyObject> for PyWrap<visualacuity::$name> {
            fn into_py(self, py: Python<'_>) -> PyObject {
                py_call(py, stringify!($name), (format!("{:?}", self.0), ))
            }
        }
    }
}
...
impl IntoPy<PyObject> for PyWrap<visualacuity::Visit> {
    fn into_py(self, py: Python<'_>) -> PyObject {
        let dict: BTreeMap<_, _> = self.0.into_iter()
            .map(|(key, entry)| (key, entry.map(PyWrap)))
            .collect();
        py_call(py, "Visit", (dict, ))
    }
}
...
impl IntoPy<PyObject> for PyWrap<visualacuity::Fraction> {
    fn into_py(self, py: Python<'_>) -> PyObject {
        ...
        py_call(py, "SnellenFraction", (distance, row, ))
    }
}

impl IntoPy<PyObject> for PyWrap<visualacuity::VisitNote> {
    fn into_py(self, py: Python<'_>) -> PyObject {
        py_call(py, "VisitNote", PyTuple::new(py, vec![
...
fn py_call(py: Python<'_>, cls: &str, args: impl IntoPy<Py<PyTuple>>) -> PyObject {
    let module = py.import("visualacuity").expect("boilerplate");
    let t = module.getattr(cls).expect("boilerplate");
    t.call1(args).expect("boilerplate").clone().into()
}

fn mapped<T, U: From<T>, E: Debug>(result: Result<T, E>) -> PyResult<U> {
    match result {
        Ok(v) => Ok(v.into()),
